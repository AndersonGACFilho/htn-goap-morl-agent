...
\begin{tikzpicture}[
    ...
    \node[htn, dashed, left=45mm of mask] (baseline)
        {Implemented Baseline\\\texttt{MethodSelectionStrategy}\\DFS / heuristic / RL hook};
    \draw[arrow, htnblue, dashed] (mask.west) --
        node[above, align=center] {orders feasible\\methods} (baseline.east);
    ...
    \draw[arrow, htnblue, dashed, rounded corners] (baseline.south)
        |- node[pos=.25, left, align=right]
        {ordered exploration\\and backtracking} (network.west);
...
As linhas tracejadas mostram o baseline já implementado: uma
\texttt{MethodSelectionStrategy} ordena métodos aplicáveis e o planner explora
os candidatos com backtracking. O caminho em vermelho permanece futuro: MORL
seleciona diretamente um único método $M_t^*$ condicionado por $WS_t$ e
$\mathbf{w}_t$.}
