...
regras, como máquinas de estados, Behavior Trees, GOAP e HTN. Essas técnicas
...
manualmente. Em ambientes dinâmicos, o número de condições necessárias para
representar todas as variações de contexto pode crescer rapidamente.
...
pode exigir grandes volumes de interação e reduz a transparência da decisão. O
MORL acrescenta a possibilidade de representar explicitamente objetivos
...
válido; um Gerador de Pesos Orientado a Objetivos define o que é importante no
contexto atual; e o MORL escolhe qual método HTN válido deve ser usado para
decompor a tarefa composta corrente.

\section{Problema de pesquisa}

Planejadores HTN tradicionais selecionam métodos por ordem fixa, custos
manuais ou heurísticas estáticas. Embora previsíveis, esses mecanismos podem
ser inadequados quando a melhor decomposição depende de saúde, munição,
condição de aliados, força inimiga, perigos, objetivo da missão, personalidade
ou eventos ocorridos durante a execução.

Uma política puramente aprendida, por outro lado, pode selecionar
comportamentos inválidos, incoerentes ou difíceis de explicar. O problema
investigado é, portanto, como introduzir adaptação multiobjetivo na escolha de
métodos HTN sem perder a validade semântica, a interpretabilidade e o controle
de designer fornecidos pelo planejamento simbólico.
...
validade semântica, interpretabilidade e desempenho de decisão em tempo real?
\end{quote}
...
\item Uma única política pode adaptar decisões a diferentes vetores de preferência sem retreinamento?
...
O foco está no nível hierárquico de decisão. A ação da política MORL é um
método de decomposição HTN, não um comando motor. Movimento, mira, animação,
desvio de obstáculos, pathfinding e execução individual de ataques serão
realizados por controladores determinísticos ou sistemas tradicionais de
jogos.
...
tarefa e três a cinco objetivos. Coordenação multiagente e aprendizado do
próprio gerador de pesos ficam fora da contribuição principal e poderão ser
considerados somente após a validação da arquitetura básica.
