...
ao longo do tempo \cite{sutton2018}.
...
representados.
Por isso, a base teórica em bandits, Processos de Decisão de Markov, funções de
valor, diferença temporal e aprendizado por reforço multiobjetivo é importante
para as etapas futuras do trabalho.
...
estruturas discretas e interpretáveis \cite{ghallab2004}.
...
e restrições.
Para uma tarefa composta $C_t$ e estado simbólico $WS_t$, o conjunto de métodos
aplicáveis é:

\begin{equation}
\mathcal{M}_{\mathrm{valid}}(C_t,WS_t)
=
\left\{M \in \mathcal{M}(C_t) \mid \mathrm{pre}(M,WS_t)=\mathrm{true}\right\}.
\end{equation}
...
\section{Planejamento HTN multiobjetivo}

Em planejamento HTN multiobjetivo, a qualidade de um plano $\pi$ não é um
escalar, mas um vetor de métricas a minimizar:

\begin{equation}
\mathbf{F}(\pi) = [f_1(\pi),f_2(\pi),\dots,f_d(\pi)].
\end{equation}

Li et al.\ formalizam o problema como
$P^*=\langle s_0,\omega_0,O,M,\mathbf{F}\rangle$, em que $s_0$ é o estado
inicial, $\omega_0$ a rede de tarefas inicial, $O$ o conjunto de operadores,
$M$ o conjunto de métodos e $\mathbf{F}$ as métricas do plano
\cite{li2024mohtn}. Para critérios de minimização, um plano $\pi_1$ domina
$\pi_2$ quando não é pior em nenhum critério e é estritamente melhor em ao
menos um:

\begin{equation}
\pi_1 \succ \pi_2
\iff
\left(\forall i,\ f_i(\pi_1) \leq f_i(\pi_2)\right)
\land
\left(\exists j,\ f_j(\pi_1) < f_j(\pi_2)\right).
\end{equation}

O algoritmo DSA (\textit{domination-based search approach}) combina duas
etapas de planejamento. Primeiro, a busca heurística estima para cada método
aplicável $m$ uma avaliação vetorial $h(m,s,t)$, obtida pela decomposição da
tarefa $t$ no estado $s$. Métodos cujos planos estimados são dominados recebem
menor prioridade; formalmente, $m_1 \succ m_2$ quando o plano estimado por
$m_1$ domina o plano estimado por $m_2$. A mesma ideia é aplicada aos
operadores de tarefas primitivas.

Segundo, a busca \textit{anytime} mantém um conjunto $\Omega$ de planos não
dominados e poda ramos cujo plano parcial é dominado por uma solução já
encontrada. Quando surge um novo plano $\pi$, a atualização pode ser expressa
como:

\begin{equation}
\Omega \leftarrow
\left\{\rho \in \Omega \cup \{\pi\}
\mid \not\exists\rho' \in \Omega \cup \{\pi\}: \rho' \succ \rho\right\}.
\end{equation}

Assim, o planejador pode devolver o conjunto corrente se for interrompido e,
com mais tempo, continuar a buscar planos não dominados adicionais. O artigo
compara o DSA com SHOP2 e com um algoritmo de busca baseado em preferências
(PSA): enquanto o PSA repete a busca para diferentes pesos e pode podar
alternativas não dominadas sob uma escalarização fixa, o DSA usa dominância
vetorial para preservar compromissos distintos entre objetivos.

Esse baseline não emprega aprendizado por reforço: não há política, função de
valor, recompensa observada ou atualização por experiência. A diferença para a
proposta deste trabalho é, portanto, algorítmica e operacional. O DSA explora
o espaço de planos no instante do planejamento e retorna um conjunto
multiobjetivo para escolha posterior; a arquitetura proposta filtra métodos
HTN válidos e usa MORL para estimar valores vetoriais e selecionar um método
sob o vetor de preferências corrente. A hipótese não é de superioridade geral
de tempo, mas de que a avaliação aprendida possa amortizar parte da escolha
online após treinamento, mantendo o HTN responsável pela validade semântica.

\section{Aprendizado de valores para métodos HTN}

Hogg, Kuter e Muñoz-Avila modelam a seleção de um método aplicável para
decompor uma tarefa HTN como uma decisão de aprendizado por reforço
\cite{hogg2010}.
Sua função escalar $Q(t,m)$ estima o retorno esperado de iniciar a
decomposição da tarefa $t$ pelo método $m$.
O estado participa indiretamente, pois apenas métodos aplicáveis no estado
corrente podem ser escolhidos.
Os autores adotam essa abstração por ser impraticável armazenar valores
específicos para todos os estados, mas ela não representa explicitamente a
dependência do valor do método em relação ao estado quando os mesmos métodos
permanecem aplicáveis em contextos distintos.

Bahrami propõe um antecedente mais próximo da interface de implementação:
uma camada de Deep Q-Learning seleciona métodos HTN aplicáveis a partir de
uma máscara de validade, mantendo o retorno escalar e o planner simbólico
\cite{bahrami2025}.
Esses trabalhos motivam baselines escalares, mas não tratam a seleção
condicionada simultaneamente pelo estado, por preferências e por um retorno
vetorial.
...
Trabalhos sobre pesos dinâmicos, \textit{successor features} e transferência de
políticas fornecem referências para essa formulação
\cite{abels2019,barreto2017successor,borsa2019usfa,barreto2020fast,alegre2022ols,alegre2026gpi}.

Para cada método válido, a política estima um vetor de valor:

\begin{equation}
\mathbf{Q}_{\theta}(C_t,WS_t,M,\mathbf{w}_t)
=
\left[Q_1,Q_2,\dots,Q_d\right]\in\mathbb{R}^d.
\end{equation}

Aqui, $C_t$ é a tarefa composta corrente, $WS_t$ é o estado simbólico relevante,
$M$ é um método candidato, $d$ é o número de objetivos e cada $Q_i$ estima o
retorno descontado do objetivo $i$ ao selecionar $M$. O vetor de preferências
satisfaz $\mathbf{w}_t\in\Delta^{d-1}$ no caso de escalarização linear
normalizada.
A dependência explícita de $\mathbf{w}_t$ permite à aproximação aprender valores
ou políticas condicionadas pela preferência, sem transferir ao HTN a decisão de
compromisso entre objetivos.
A recompensa vetorial não é alterada pelos pesos: a dependência em
$\mathbf{w}_t$ decorre da política de continuação condicionada por preferência,
que pode produzir trajetórias e retornos vetoriais esperados diferentes.

Sob escalarização linear, a seleção é:

\begin{equation}
 M_t^* =
 \arg\max_{M\in \mathcal{M}_{\mathrm{valid}}(C_t,WS_t)}
 \mathbf{w}_t^{\top}
 \mathbf{Q}_{\theta}(C_t,WS_t,M,\mathbf{w}_t).
\end{equation}

No primeiro estágio experimental, a utilidade será restrita à escalarização
linear, com $w_i\geq 0,\qquad \sum_i w_i=1$.
Sob essa hipótese, os critérios \textit{Scalarised Expected Returns} (SER) e
\textit{Expected Scalarised Returns} (ESR) coincidem.
Utilidades não lineares, preferências lexicográficas e a distinção entre SER
e ESR permanecem fora do escopo inicial \cite{hayes2022morl}.
...
podem, contudo, inspirar uma representação de contexto usada por uma fonte de
preferências.

Uma fonte de preferências produz ou fornece diretamente um vetor normalizado de
preferências a partir do contexto disponível $\xi_t$:

\begin{equation}
\mathbf{w}_t = \Phi(\xi_t),
\qquad
w_{i,t}\geq 0,
\qquad
\sum_i w_{i,t}=1.
...
Nessa expressão, $\Phi$ é uma \textit{PreferenceSource}, $\xi_t$ representa o
contexto disponível e $w_{i,t}$ é o peso do objetivo $i$ no instante $t$. Para
fontes diretas, $\Phi$ pode ser uma tabela, perfil ou regra; para fontes
aprendidas, pode ser um codificador parametrizado.
...
essa extensão.
Assim, a comparação entre fontes de preferência é uma parte auxiliar da pesquisa, enquanto
a contribuição central permanece a seleção MORL de métodos HTN válidos.
Em um contexto mais amplo de integração entre planejamento e aprendizado, uma
base de políticas aprendidas pode apoiar a composição de soluções para tarefas
complexas \cite{kuric2024}, e planejadores relacionais podem atuar em conjunto
com RL para favorecer eficiência amostral, transferência e generalização
\cite{prabhakar2025}. Esses trabalhos reforçam a pertinência de arquiteturas
híbridas, sem resolver diretamente a seleção MORL de métodos HTN condicionada
por preferências investigada nesta proposta.

\section{Decisão hierárquica e abstração temporal}

A literatura de aprendizado por reforço hierárquico organiza abstrações temporais
em múltiplos mecanismos de descoberta e composição \cite{klissarov2025hrl}.
Trabalhos sobre \textit{Option Keyboard} mostram como habilidades temporalmente
estendidas podem ser combinadas a partir de uma base de comportamentos
\cite{barreto2019optionkeyboard,alegre2025optionkeyboard}. Esses antecedentes
motivam a abstração da proposta, sem identificar métodos HTN diretamente com
\textit{options}.
...
Para treinamento de valor de longo prazo, o retorno acumulado é apenas o
primeiro termo de uma atualização semi-Markoviana.
Distinguindo $t$ como o tempo primitivo e $j$ como o índice da decisão HTN,
seja $t_j$ o início da decisão, $t_{j+1}$ o instante da próxima decisão
hierárquica ou do término do episódio, e $k_j=t_{j+1}-t_j$.
Então:

\begin{equation}
\mathbf{R}^{\mathrm{method}}_j =
\sum_{i=0}^{k_j-1}\gamma^i\mathbf{r}_{t_j+i},
\qquad
\mathbf{y}_j =
\mathbf{R}^{\mathrm{method}}_j +
\gamma^{k_j}\mathbf{V}_{\mathrm{cont}}(C_{j+1},WS_{t_{j+1}},\mathbf{w}_{j+1}),
\end{equation}

Para transições não terminais, $\mathbf{V}_{\mathrm{cont}}$ denota a estimativa
vetorial de continuação definida pelo algoritmo MORL selecionado. Em uma
implementação baseada em Q-learning/DQN, ela pode ser estimada por
$\mathbf{Q}_{\theta^-}(C_{j+1},WS_{t_{j+1}},M^*_{j+1},\mathbf{w}_{j+1})$, em que
$M^*_{j+1}$ é obtido pelo \textit{argmax} mascarado da política.
Em terminais, $\mathbf{y}_j=\mathbf{R}^{\mathrm{method}}_j$.
Essa formulação é compatível com a atualização SMDP para decisões
temporalmente estendidas \cite{sutton1999options}; ela não pressupõe que um método
HTN seja uma \textit{Option}, pois o método permanece uma decomposição simbólica
