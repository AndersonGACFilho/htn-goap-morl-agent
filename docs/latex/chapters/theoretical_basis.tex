...
\longrightarrow \mathcal{M}_{\mathrm{valid}}(C_t,WS_t),\\
(\text{jogo},\text{diretor},\text{perfil},\text{regra},C_t,WS_t)
&\longrightarrow \text{fonte de preferências}
\longrightarrow \mathbf{w}_t,\\
(\mathcal{M}_{\mathrm{valid}},C_t,WS_t,\mathbf{w}_t)
&\longrightarrow \text{MORL}
\longrightarrow M_t^*.
\end{align}

A fonte de preferências é obrigatória como interface de fornecimento de
$\mathbf{w}_t$; codificadores relacionais ou aprendidos são implementações
opcionais dessa interface.
...
Como contraste, Li et al.\ resolvem otimização multiobjetivo HTN por busca
heurística e \textit{anytime}, comparando planos parciais por dominância e
mantendo um conjunto de planos não dominados \cite{li2024mohtn}.
A proposta presente não assume ser mais rápida: sua hipótese é que a avaliação
aprendida possa amortizar parte desse trabalho, deslocando exploração para
treinamento e reduzindo o trabalho no caminho normal da decisão online.

\input{figures/htn_morl_architecture}
