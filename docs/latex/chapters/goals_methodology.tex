...
Em cada ponto de decisão, a entrada da política será especificada como
$s_j=(WS_j,C_j,\phi(o_j),\mathbf{m}_j,\mathbf{w}_j)$, em que $WS_j$ é o
estado simbólico, $C_j$ é a tarefa composta, $\phi(o_j)$ é um vetor de
observação de dimensão fixa, $\mathbf{m}_j$ é a máscara de métodos e
$\mathbf{w}_j$ é a preferência normalizada. No cenário inicial,
$\phi$ será uma codificação tabular ou vetorial documentada; representações
com cardinalidade variável, como grafos, serão tratadas como extensão e
exigirão codificador relacional explicitamente avaliado.
...
O protocolo combinará um conjunto finito de perfis interpretáveis com
amostras de uma distribuição de Dirichlet sobre o simplex; seus parâmetros,
sementes e vetores reservados para teste serão fixados antes do treinamento.
...
replanejamento.
Cada transição registrada conterá $(s_j,M_j,\mathbf{R}_j,k_j,s_{j+1},z_j)$,
em que $k_j$ é o número de passos primitivos e $z_j$ identifica o motivo de
término. Atualizações semi-MDP aplicarão o desconto $\gamma^{k_j}$; o
repositório de experiências preservará $k_j$ e o traço primitivo para permitir
comparar retornos de $n$ passos e diferentes regras de truncamento sob durações
variáveis.
...
os métodos. Essa falha também gerará uma transição explícita, sem passos
primitivos, marcada como inviabilidade de decomposição. Seu custo de planejamento
será registrado e, se usado no treinamento, entrará como componente negativo
documentado da recompensa vetorial; desse modo, a política recebe crédito pela
falha sem confundi-la com uma transição do ambiente.
