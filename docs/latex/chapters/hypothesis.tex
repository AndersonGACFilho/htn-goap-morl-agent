...
\item[H6 --- codificação de preferências:] sob a mesma política MORL e a mesma
máscara HTN, um codificador relacional de preferências poderá ser comparado de
forma controlada a perfis fixos e regras explícitas; um codificador profundo só
será considerado se essa comparação justificar maior complexidade.
\end{description}
...
Para cada decisão, ele produz $M_{\mathrm{valid}}(s_t,\tau_t)$.
Métodos impossíveis ou proibidos são eliminados antes da consulta ao modelo aprendido.

\subsection{Codificador de preferências}

O codificador produz $\mathbf{w}_t$ a partir do objetivo, estado, perfil e
contexto.
...
\item o codificador de preferências fornece $\mathbf{w}_t$;
